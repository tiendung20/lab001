...
std::string LatencyEmergencyTime::getName() {
    return "Objective: Threshold - " + std::to_string(Constant::THRESHOLD)
            + "Trade-off Latency & EmergencyTime  =>"
            + std::to_string(Constant::EMERGENCY_TIME / 60);
}
...
double LatencyEmergencyTime::getHarmfulnessArrival(AGV *cur, double time) {
    ...
    if (bestTime + count * period - amplitude <= time
            && bestTime + count * period + amplitude >= time) {
        return 0;
    }
    if (bestTime + count * period - amplitude > time) {
...
void LatencyEmergencyTime::planOut( //std::vector <Quad> adjList[],
    ...
    this->cur->ShortestPath[source] = this->getHarmfulnessArrival(this->cur,
            ratio * firstCost + now);

    this->cur->PQ.push(
            std::make_tuple(this->cur->ShortestPath[source], 0, source, " 0")); // Source has weight cur->ShortestPath[source];
    bool found = false;
    ...
    while (!this->cur->PQ.empty()) {
        ...
        if (source == target) {
            ...
            if (target != this->cur->itinerary->exit
                    || this->cur->itinerary->exit
                            == this->cur->itinerary->indexStation) {
                ...
                if (totalCost
                        < Constant::THRESHOLD|| cur->MIN_EMERGENCY == DBL_MAX) {
                    //the cur->MIN_EMERGENCY == DBL_MAX to ensure that at least one path will be added
                    accept = true;
                    found = true;
                }
                if (accept && cur->traces[target].compare(last) != 0) {
                    ...
                    this->cur->allMinOptions.push_back(
                            std::make_tuple(totalEmergency, std::get<1>(info),
                                    target, last)); // Source has weight cur->ShortestPath[source];
    ...
    if (found) {
        ...
        cur->expectedTimeAtStation = ratio
                * std::get<1>(this->cur->allMinOptions[index]) + cur->now;
...
void LatencyEmergencyTime::checkActiveEdges(double firstCost, Quad *info,
        bool activeEdges) {
    std::string trace;
    ...
    std::vector<std::vector<Quad>> *list =
            activeEdges ? &adjList : &emergencyAdjList;
    for (std::vector<Quad>::iterator it = (*list)[source].begin();
            it != (*list)[source].end(); it++) {
        ...
        newObjective = this->getHarmfulnessArrival(cur,
                ratio * (newWeight) + now);
        ...
        if (newObjective < cur->ShortestPath[tempIndex]
                || newObjective < Constant::THRESHOLD) { // Check if we can do better
            ...
            cur->PQ.push(
                    make_tuple(newObjective, newWeight, /*content,*/tempIndex,
                            newTrace));
        } // Update distance
    } //End of for
}
