...
enum HeadOrTail {
    Head = 0, Normal = 1, Tail = 1000, Both = 2
};

enum TypeOfZone {
    ZoneType = 0, EmptyType = 1, UsingType = 2
};
...
#pragma endregion
        if (key <= anchor0) {
            ...
            newKey = key - (anchor5 - anchor4 - 1) - (anchor3 - anchor2 - 1)
                    - (anchor1 - anchor0 - 1);
        } else if (key >= anchor7 && key <= anchor8) {
            newKey = key //- (anchor7a - anchor6a - 1)
            - (anchor7 - anchor6 - 1) - (anchor5 - anchor4 - 1)
                    - (anchor3 - anchor2 - 1) - (anchor1 - anchor0 - 1);
        } else if (key >= anchor9 && key <= anchor10) {
            newKey = key - (anchor9 - anchor8 - 1) - (anchor7 - anchor6 - 1)
                    - (anchor5 - anchor4 - 1) - (anchor3 - anchor2 - 1)
                    - (anchor1 - anchor0 - 1);
        } else if (key >= anchor11 && key <= anchor12) {
            newKey = key - (anchor11 - anchor10 - 1) - (anchor9 - anchor8 - 1)
                    - (anchor7 - anchor6 - 1) - (anchor5 - anchor4 - 1)
                    - (anchor3 - anchor2 - 1) - (anchor1 - anchor0 - 1);
        } else if (key >= anchor13) {
            newKey = key - (anchor13 - anchor12 - 1) - (anchor11 - anchor10 - 1)
                    - (anchor9 - anchor8 - 1) - (anchor7 - anchor6 - 1)
                    - (anchor5 - anchor4 - 1) - (anchor3 - anchor2 - 1)
                    - (anchor1 - anchor0 - 1);
        }
        return newKey;
...
private:
    ...
    double getLength(const char *content) {
        ...
        if (found != -1 && isdigit(content[found + 1])) { //found the start of lane's length
            for (int i = found; i < strlen(content); i++) {
                if (content[i] == '.'
                        || isdigit(content[i]) && index < NUM_LENGTH) {
                    num[index] = content[i];
                    index++;
                }
            }
    ...
    void update(GLane *lane, std::string tp) {
        ...
        if (indexesOfNeighbors[0] != -1)
            lane->numNeighbors++;
        if (indexesOfNeighbors[1] != -1)
            lane->numNeighbors++;
        if (indexesOfNeighbors[2] != -1)
            lane->numNeighbors++;
    }
            ...
            for (int j = 0; j < 3; j++) {
                if (allLanes[i]->neighborsIds[j] != -1) {
                    ...
                    if (indexOfNeighbor < this->N) {
                        if (allLanes[allLanes[i]->neighborsIds[j]]->from[0]
                                == -1) {
                            allLanes[indexOfNeighbor]->from[0] = i;
                        } else if (allLanes[allLanes[i]->neighborsIds[j]]->from[1]
                                == -1) {
                            allLanes[allLanes[i]->neighborsIds[j]]->from[1] = i;
                        } else if (allLanes[allLanes[i]->neighborsIds[j]]->from[2]
                                == -1) {
                        ...
                        } else if (allJuncs[idOfJ]->allLanes[offset]->from[1]
                                == -1) {
                            allJuncs[idOfJ]->allLanes[offset]->from[1] = i;
                        } else if (allJuncs[idOfJ]->allLanes[offset]->from[2]
                                == -1) {
            ...
            if (allJuncs[i] != NULL) {
                for (int j = 0; j < 16; j++) {
                    if (allJuncs[i]->allLanes[j] != NULL) {
                        for (int k = 0; k < 3; k++) {
                            if (allJuncs[i]->allLanes[j]->neighborsIds[k]
                                    != -1) {
                                int indexOfNeighbor =
                                        allJuncs[i]->allLanes[j]->neighborsIds[k];
                                if (indexOfNeighbor < this->N) {
                                    if (allLanes[indexOfNeighbor]->from[0]
                                            == -1) {
                                        allLanes[indexOfNeighbor]->from[0] = ((i
                                                + this->N) | (j << 16));
                                    } else if (allLanes[indexOfNeighbor]->from[1]
                                            == -1) {
                                        allLanes[indexOfNeighbor]->from[1] = ((i
                                                + this->N) | (j << 16));
                                    } else if (allLanes[indexOfNeighbor]->from[2]
                                            == -1) {
                                        allLanes[indexOfNeighbor]->from[2] = ((i
                                                + this->N) | (j << 16));
                                    }
                                } else {
                                    int offset = 0;
                                    int idOfJ = extractLaneOfJunc(
                                            indexOfNeighbor, &offset);
                                    if (allJuncs[idOfJ]->allLanes[offset]->from[0]
                                            == -1) {
                                        allJuncs[idOfJ]->allLanes[offset]->from[0] =
                                                ((i + this->N) | (j << 16));
                                    } else if (allJuncs[idOfJ]->allLanes[offset]->from[1]
                                            == -1) {
                                        allJuncs[idOfJ]->allLanes[offset]->from[1] =
                                                ((i + this->N) | (j << 16));
                                    } else if (allJuncs[idOfJ]->allLanes[offset]->from[2]
                                            == -1) {
                                        allJuncs[idOfJ]->allLanes[offset]->from[2] =
                                                ((i + this->N) | (j << 16));
                                    }
                                }
                            }
    ...
    void updateHeadOfTail(GLane *lane, int size) {
        ...
        if (size == 1)
            lane->zones[0]->_hOrT = HeadOrTail::Both;
    }
};
