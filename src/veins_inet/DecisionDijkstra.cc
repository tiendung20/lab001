...
void DecisionDijkstra::generateEmergencyEdges() {
    ...
    for (int i = 0; i < this->numIVertices; i++) {
        for (std::vector<Quad>::iterator it = this->adjList[i].begin();
                it != this->adjList[i].end(); it++) {
            ...
            this->emergencyAdjList[i].push_back(
                    std::make_tuple(tempW, tempW, tempIndex, newTempTrace));
        }
    }
}
...
void DecisionDijkstra::planOut( //std::vector <Quad> adjList[],
    ...
    this->cur->PQ.push(
            std::make_tuple(this->cur->ShortestPath[source], 0, source, "")); // Source has weight cur->ShortestPath[source];
...
void DecisionDijkstra::checkActiveEdges(double firstCost, Quad *info,
        bool activeEdges) {
    std::string trace;
...
double DecisionDijkstra::getHarmfulnessAvailable(double time,
        std::string tempTrace) {
