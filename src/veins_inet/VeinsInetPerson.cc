...
void VeinsInetPerson::initialize(int stage)
{
    ...
    if (stage == 0) {
        ...
        if(sendBeacon != NULL){
            ...
            //if(myId == Constant::WANTED_ID)
            {
                ...
                EV<<"Initialize Person"<<myId<<" at "<<simTime().dbl()<<" ";
                //curPosition = mobility->getCurrentDirection();
                //EV<<curPosition.x<<":"<<curPosition.y<<endl;
                //EV<<traciVehicle->getLanePosition()<<endl;
            }
        }
    }
...
void VeinsInetPerson::onBSM(DemoSafetyMessage* bsm)
{
    //for my own simulation circle
}

void VeinsInetPerson::onWSM(BaseFrame1609_4* wsm)
{
    // Your application has received a data message from another car or RSU
    // code for handling the message goes here, see TraciDemo11p.cc for examples
}

void VeinsInetPerson::onWSA(DemoServiceAdvertisment* wsa)
{
    // Your application has received a service advertisement from another car or RSU
    // code for handling the message goes here, see TraciDemo11p.cc for examples
}

void VeinsInetPerson::handleSelfMsg(cMessage* msg)
{
...
void VeinsInetPerson::handlePositionUpdate(cObject* obj)
{
...
void VeinsInetPerson::handleLowerMsg(cMessage* msg)
{
