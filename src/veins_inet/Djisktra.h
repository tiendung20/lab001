...
enum STATE_OF_REQUEST {
    WAITING_FOR_PROCESSING, BEING_PROCESSED, FINISHED
};
...
public:
    ...
    double exponentialSmooth(int index, double oldPredict) {
        ...
        if (k[index] == 0) {
            ...
            Qt[index] = Constant::GAMMA * error
                    - (1 - Constant::GAMMA) * Qt[index];
    ...
    void printMaxWeights(std::vector<std::string> vertices) {
        for (int i = 0; i < this->num; i++) {
            if (maxWeights[i] > 0.001)
                EV << vertices[i] << " has max: " << maxWeights[i] << " at "
                          << timeOfPeaks[i] << endl;
        }
    }
    ...
    double getDampingValue(int index, double predictW, std::string nameVertex) {
        ...
        if (!checkPedestrians) {
            if (simTime().dbl() - timeOflastSmoothing
                    > Constant::EXPIRED_TIME) {
                if (waitTime[index] > 0) {
    ...
    double useCycicalData(double incommingTime, std::string name,
            double weightSmoothing) {
        if (incommingTime > this->beginWalking
                && incommingTime < this->endWalking) {
            if (allNoisyIntersections.find("$" + name + "$")
                    != std::string::npos) {
                return 0.5;
            }
        }
...
public:
    ...
    virtual bool insertRequest(int source, int dst, std::string id) {
        if (!isWorking()) {
            ...
            for (std::vector<Request>::iterator it = allRequests.begin();
                ...
                if (theSource == source && theDestination == dst) {
                    ...
                    if (ids.compare("$" + id + "$") == 0) {
                        ...
                        std::string newIds = ids.replace(start,
                                replacedStr.length(), "");
                        std::get<2>(*it) = newIds;
                    }
                }
            ...
            allRequests.push_back(
                    std::make_tuple(source, dst, "$" + id + "$", t,
    ...
    virtual bool canExecuteReqs() {
        ...
        for (std::vector<Request>::iterator it = allRequests.begin();
            ...
            if (state == BEING_PROCESSED) {
                return false;
            } else if (state == WAITING_FOR_PROCESSING
                    && (t - createdTime < Constant::DELAY)) {
                waiting++;
            }
        }
    ...
    virtual bool hasIdenticalReq() {
        ...
        for (std::vector<Request>::iterator it = allRequests.begin();
            ...
            int theSource = std::get<0>(*it);
            int theDestination = std::get<1>(*it);
            for (std::vector<Request>::iterator ptr = allRequests.begin() + i;
                    ptr != allRequests.end(); ptr++) {
    ...
    virtual void initAdaptiveEdges(std::vector<std::vector<Quad>> adjList,
            std::vector<std::string> vertices,
            std::vector<double> timeWeightVertices, int *countEdges) //prepare for parallelization
            {
        double tempW;
        int tempIndex;
        ...
        for (int i = 0; i < numVertices; i++) {
            for (std::vector<Quad>::iterator it = adjList[i].begin();
                    it != adjList[i].end(); it++) {
                AdaptiveSystem::Edge edge;
                tempW = std::get<0>(*it);
                if (tempW == 0) {
                ...
                edge.edgeStart = i;
                edge.weightSrc = std::round(timeWeightVertices[i] * 100.0)
                        / 100.0;
                edge.edgeEnd = tempIndex;
                edge.weightDst = std::round(
                        timeWeightVertices[tempIndex] * 100.0) / 100.0;
...
public:
    ...
    virtual std::string getRoute(std::string trace, std::string currentLane,
    ...
    double timeForVeryNextVertex(std::string currLane,
